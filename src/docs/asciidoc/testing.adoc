...
set up com.mxk.service locator registries and similar structures).
...
you can test com.mxk.service layer objects by stubbing or mocking DAO or repository interfaces,
...
* ORM frameworks (such as JPA and Hibernate) that condone `private` or `protected` field
  access as opposed to `public` setter methods for properties in a domain com.mxk.entity.
...
* Data access using JDBC or an ORM tool. This can include such things as the correctness
  of SQL statements, Hibernate queries, JPA com.mxk.entity mappings, and so forth.
...
that implements data access logic for a `Title` domain com.mxk.entity. We want to write
...
.Java
----
	@ContextConfiguration("/test-com.mxk.config.xml") // <1>
...
----
	@ContextHierarchy({
		@ContextConfiguration("/parent-com.mxk.config.xml"),
		@ContextConfiguration("/child-com.mxk.config.xml")
	})
...
----
	...
	@Sql(
		...
		com.mxk.config = @SqlConfig(commentPrefix = "`", separator = "@@") // <1>
	)
	void userTest() {
...
----
	@Test
	@Sql("/test-user-data.sql", com.mxk.config = SqlConfig(commentPrefix = "`", separator = "@@")) // <1>
	fun userTest() {
		// run code that relies on the test data
	}
----
...
----
	@Test
	@SqlGroup({ // <1>
		@Sql(scripts = "/test-schema.sql", com.mxk.config = @SqlConfig(commentPrefix = "`")),
...
----
	@Test
	@SqlGroup( // <1>
		Sql("/test-schema.sql", com.mxk.config = SqlConfig(commentPrefix = "`")),
		Sql("/test-user-data.sql"))
	fun userTest() {
		// run code that uses the test schema and test data
	}
...
.Java
----
	@SpringJUnitConfig(locations = "/test-com.mxk.config.xml") // <1>
...
.Java
----
	@SpringJUnitWebConfig(locations = "/test-com.mxk.config.xml") // <1>
...
----
	...
	@ContextConfiguration({"/app-com.mxk.config.xml", "/test-data-access-com.mxk.config.xml"})
	@ActiveProfiles("dev")
	@Transactional
...
the `TestContextManager` with which the listener is registered. See <<testcontext-tel-com.mxk.config>>.
...
[[testcontext-tel-com.mxk.config]]
...
[[testcontext-tel-com.mxk.config-registering-tels]]
...
[[testcontext-tel-com.mxk.config-automatic-discovery]]
...
[[testcontext-tel-com.mxk.config-ordering]]
...
through the <<testcontext-tel-com.mxk.config-automatic-discovery, aforementioned>>
...
[[testcontext-tel-com.mxk.config-merging]]
...
implementations by using the aforementioned <<testcontext-tel-com.mxk.config-automatic-discovery,
...
of `AnnotationAwareOrderComparator`, as described in <<testcontext-tel-com.mxk.config-ordering>>.
...
* <<testcontext-ctx-management-mixed-com.mxk.config>>
...
is treated as an absolute classpath location (for example, `/org/example/com.mxk.config.xml`). A
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from "/app-com.mxk.config.xml" and
	// "/test-com.mxk.config.xml" in the root of the classpath
	@ContextConfiguration(locations={"/app-com.mxk.config.xml", "/test-com.mxk.config.xml"}) // <1>
	class MyTest {
		// class body...
	}
----
...
----
	...
	// ApplicationContext will be loaded from "/app-com.mxk.config.xml" and
	// "/test-com.mxk.config.xml" in the root of the classpath
	@ContextConfiguration("/app-com.mxk.config.xml", "/test-com.mxk.config.xml") // <1>
	class MyTest {
		// class body...
	}
----
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from
	// "/app-com.mxk.config.xml" and "/TestConfig.groovy"
	@ContextConfiguration({ "/app-com.mxk.config.xml", "/TestConfig.groovy" })
	class MyTest {
		// class body...
	}
----
...
`ExtendedTest` is loaded from `base-com.mxk.config.xml` and `extended-com.mxk.config.xml`, in that order.
Beans defined in `extended-com.mxk.config.xml` can, therefore, override (that is, replace) those
defined in `base-com.mxk.config.xml`. The following example shows how one class can extend
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from "/base-com.mxk.config.xml"
	// in the root of the classpath
	@ContextConfiguration("/base-com.mxk.config.xml") <1>
	class BaseTest {
		// class body...
	}

	// ApplicationContext will be loaded from "/base-com.mxk.config.xml" and
	// "/extended-com.mxk.config.xml" in the root of the classpath
	@ContextConfiguration("/extended-com.mxk.config.xml") <2>
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean id="transferService"
				class="com.bank.com.mxk.service.internal.DefaultTransferService">
		...
		<bean id="feePolicy"
			class="com.bank.com.mxk.service.internal.ZeroFeePolicy"/>

		<beans profile="dev">
			<jdbc:embedded-database id="dataSource">
				<jdbc:script
					location="classpath:com/bank/com.mxk.config/sql/schema.sql"/>
				<jdbc:script
					location="classpath:com/bank/com.mxk.config/sql/test-data.sql"/>
...
----
	@ExtendWith(SpringExtension.class)
	// ApplicationContext will be loaded from "classpath:/app-com.mxk.config.xml"
	@ContextConfiguration("/app-com.mxk.config.xml")
	@ActiveProfiles("dev")
...
`app-com.mxk.config.xml` configuration file in the root of the classpath. If you inspect
`app-com.mxk.config.xml`, you can see that the `accountRepository` bean has a dependency on a
	...
	public class StandaloneDataConfig {
		...
		public DataSource dataSource() {
			return new EmbeddedDatabaseBuilder()
				...
				.addScript("classpath:com/bank/com.mxk.config/sql/schema.sql")
				.addScript("classpath:com/bank/com.mxk.config/sql/test-data.sql")
				.build();
		}
	}
----
...
----
	...
	// classpath resource
	@ContextConfiguration("/spring/test-servlet-com.mxk.config.xml")
	class WacTests {
		//...
	}
----
...
----
	...
	// file system resource
	@ContextConfiguration("file:src/main/webapp/WEB-INF/servlet-com.mxk.config.xml")
	class WacTests {
		//...
	}
----
...
For example, if `TestClassA` specifies `{"app-com.mxk.config.xml", "test-com.mxk.config.xml"}` for the
...
`{"app-com.mxk.config.xml", "test-com.mxk.config.xml"}` for its locations (either explicitly or
...
are loaded: one for `/app-com.mxk.config.xml`, one for `/user-com.mxk.config.xml`, and one for
`{"/user-com.mxk.config.xml", "/order-com.mxk.config.xml"}`. As with the previous example, the
application context loaded from `/app-com.mxk.config.xml` is set as the parent context for the
contexts loaded from `/user-com.mxk.config.xml` and `{"/user-com.mxk.config.xml", "/order-com.mxk.config.xml"}`.
...
application context for `ExtendedTests` is loaded only from `/test-user-com.mxk.config.xml` and
has its parent set to the context loaded from `/app-com.mxk.config.xml`. The following listing
...
----
	...
	@ContextHierarchy(
		@ContextConfiguration(
			name = "child",
			locations = "/test-user-com.mxk.config.xml",
...
----
	...
	@ContextConfiguration("repository-com.mxk.config.xml")
...
`@ContextConfiguration` annotation (that is, `repository-com.mxk.config.xml`). The following
...
`userService`, we are assured that the user com.mxk.service has access to the request-scoped
...
the user com.mxk.service has access to the session-scoped `userPreferences` for the current
...
[[testcontext-tx-mgr-com.mxk.config]]
...
----
	...
	@Sql(
		...
		com.mxk.config = @SqlConfig(transactionMode = ISOLATED)
	)
	@Sql(
...
declared directly by using the `com.mxk.config` attribute of the `@Sql` annotation, `@SqlConfig`
...
You can write a plain unit test for a Spring MVC com.mxk.controller by using JUnit or TestNG. To
do so, instantiate the com.mxk.controller, inject it with mocked or stubbed dependencies, and
...
Furthermore, other com.mxk.controller methods such as `@InitBinder`, `@ModelAttribute`, and
...
Your second option is to manually create a com.mxk.controller instance without loading Spring
...
a mock com.mxk.service with Mockito:
...
You can then inject the mock com.mxk.service into the test to set up and verify your
...
com.mxk.controller at a time. You can manually inject the com.mxk.controller with mock dependencies, and
it does not involve loading Spring configuration. Such tests are more focused on style
and make it easier to see which com.mxk.controller is being tested, whether any specific Spring
...
inspect Spring MVC specific aspects, such as which com.mxk.controller method processed the
...
Below is an example test for com.mxk.controller methods that return `DeferredResult`, `Callable`,
...
the stand-alone setup, focusing on one com.mxk.controller at a time and manually providing the
...
This test has some obvious drawbacks. If we update our com.mxk.controller to use the parameter
`message` instead of `text`, our form test continues to pass, even though the HTML form
is out of synch with the com.mxk.controller. To resolve this we can combine our two tests, as
follows:
