...
This section starts by exposing the com.mxk.service to a remote client by using RMI and talk a bit
...
plain RMI in case of a traditional RMI com.mxk.service. The `RmiServiceExporter` explicitly
supports the exposing of any non-RMI services via RMI invokers.

We first have to set up our com.mxk.service in the Spring container.
...
Next, we have to expose our com.mxk.service by using `RmiServiceExporter`.
The following example shows how to do so:
...
----
	<bean class="org.springframework.remoting.rmi.RmiServiceExporter">
		...
		<property name="com.mxk.service" ref="accountService"/>
...
Furthermore, the com.mxk.service name is used to bind the com.mxk.service. So, in the preceding example, the
com.mxk.service is bound at `'rmi://HOST:1199/AccountService'`. We use this URL later on to link in
the com.mxk.service at the client side.

NOTE: The `servicePort` property has been omitted (it defaults to 0). This means that an
anonymous port is used to communicate with the com.mxk.service.
...
To link in the com.mxk.service on the client, we create a separate Spring container,
to contain the following simple object and the com.mxk.service linking configuration bits:
...
That is all we need to do to support the remote account com.mxk.service on the client. Spring
transparently creates an invoker and remotely enables the account com.mxk.service through the
...
Now we are ready to link in the com.mxk.service at the client. No explicit handler mapping is
specified (to map request URLs onto services), so we use `BeanNameUrlHandlerMapping`
used. Hence, the com.mxk.service is exported at the URL indicated through its bean name
...
By using the `HessianProxyFactoryBean`, we can link in the com.mxk.service at the client. The same
...
Setting up the HTTP invoker infrastructure for a com.mxk.service object closely resembles the
...
Again, linking in the com.mxk.service from the client much resembles the way you would do it
when you use Hessian. By using a proxy, Spring can translate your calls to
HTTP POST requests to the URL that points to the exported com.mxk.service. The following example
...
----
	 ...
	 * endpoint classes. If an existing com.mxk.service needs to be exported, a wrapper that
	 ...
	 * accordingly. The servlet name usually needs to match the specified WS com.mxk.service name.
	 *
	 * The web com.mxk.service engine manages the lifecycle of instances of this class.
...
See the various Java EE web com.mxk.service tutorials for details.
...
shown earlier), you have one servlet definition per com.mxk.service endpoint, with each endpoint
...
Spring provides two factory beans to create JAX-WS web com.mxk.service proxies, namely
`LocalJaxWsServiceFactoryBean` and `JaxWsPortProxyFactoryBean`. The former can
return only a JAX-WS com.mxk.service class for us to work with. The latter is the full-fledged
version that can return a proxy that implements our business com.mxk.service interface.
...
.wsdl file. `serviceName` corresponds to the com.mxk.service name in the .wsdl file. `portName`
corresponds to the port name in the .wsdl file.

Accessing the web com.mxk.service is easy, as we have a bean factory for it that exposes it as
...
----
	<bean id="client" class="example.AccountClientImpl">
		...
		<property name="com.mxk.service" ref="accountWebService"/>
	</bean>
----

From the client code, we can access the web com.mxk.service as if it were a normal class,
as the following example shows:
...
----
	public class AccountClientImpl {
		...
		private AccountService com.mxk.service;

		public void setService(AccountService com.mxk.service) {
			this.com.mxk.service = com.mxk.service;
		}

		public void foo() {
			com.mxk.service.insertAccount(...);
		}
	}
----
...
On the server, you need to expose the com.mxk.service object that uses the
...
----
	...
	public class Client {
		...
		public static void main(String[] args) throws Exception {
			...
			CheckingAccountService com.mxk.service = (CheckingAccountService) ctx.getBean("checkingAccountService");
			com.mxk.service.cancelAccount(new Long(10));
		}
	}
----
...
in the local case. However, when you export a remote com.mxk.service, you should expose a specific
com.mxk.service interface, with specific operations intended for remote usage. Besides internal
...
com.mxk.service interface to be specified.

This is a trade-off between configuration convenience and the risk of accidental
exposure of internal methods. Always specifying a com.mxk.service interface is not too much
...
<<web.adoc#mvc-com.mxk.config-message-converters, Configuring Message Converters>>).
...
Assume that we have a web com.mxk.controller that needs to use a local EJB. We follow best
...
switch to a POJO (plain old Java object) implementation of the com.mxk.service if it makes sense
...
com.mxk.controller to the EJB implementation is to expose a setter method of type `MyComponent`
on the com.mxk.controller. This saves the reference as an instance variable in the
com.mxk.controller. The following example shows how to do so:
...
We can subsequently use this instance variable in any business method in the com.mxk.controller.
Now, assuming we obtain our com.mxk.controller object out of a Spring container, we can
...
`myComponent` property of the com.mxk.controller with the following configuration entry:
...
The `myController` bean definition sets the `myComponent` property of the com.mxk.controller
...
is a subclass of `RuntimeException`. You can then switch the target com.mxk.service at will
...
for managing JMS destinations, such as providing a com.mxk.service locator for destinations
stored in JNDI.
...
The `org.springframework.jms.com.mxk.config` package provides the parser implementation for the
`jms` namespace as well as the java com.mxk.config support to configure listener containers and
...
`JndiDestinationResolver` is also provided to act as a com.mxk.service locator for
...
in the form of a business com.mxk.entity existence check or a protocol table check.
...
----
	...
	<bean id="jmsListenerContainerFactory"
			class="org.springframework.jms.com.mxk.config.DefaultJmsListenerContainerFactory">
		<property name="connectionFactory" ref="connectionFactory"/>
...
case (through setting the 'proxy-target-class' flag on `<aop:com.mxk.config/>`,
...
`com.mxk.service:jmx:jmxmp://localhost:9875`. The `serverConnector` bean thus exposes the
...
----
	<bean id="serverConnector"
		...
		<property name="serviceUrl"
				value="com.mxk.service:jmx:rmi://localhost/jndi/rmi://localhost:1099/myconnector"/>
	</bean>
----
...
----
	<bean id="serverConnector"
		...
		<property name="serviceUrl"
			value="com.mxk.service:jmx:iiop://localhost/jndi/iiop://localhost:900/myconnector"/>
...
Note that, when you use a RMI-based connector, you need the lookup com.mxk.service (`tnameserv` or
...
----
	<bean id="clientConnector" class="org.springframework.jmx.support.MBeanServerConnectionFactoryBean">
		<property name="serviceUrl" value="com.mxk.service:jmx:rmi://localhost/jndi/rmi://localhost:1099/jmxrmi"/>
	</bean>
----
...
----
	<bean id="serverConnector" class="org.springframework.jmx.support.ConnectorServerFactoryBean">
		...
		<property name="serviceUrl" value="com.mxk.service:jmx:burlap://localhost:9874"/>
	</bean>
----
...
----
	<bean id="clientConnector"
		...
		<property name="serviceUrl" value="com.mxk.service:jmx:rmi://remotehost:9875"/>
	</bean>
...
* <<cci-com.mxk.config-connector>>
* <<cci-com.mxk.config-connectionfactory>>
* <<cci-com.mxk.config-cci-connections>>
* <<cci-com.mxk.config-single-connection>>
...
* `WorkManagerTaskExecutor`:
  This implementation uses a CommonJ `WorkManager` as its backing com.mxk.service provider
...
temporary store for data between a fast and a slow com.mxk.entity. As one party would have to wait
...
As with other services in the Spring Framework, the caching com.mxk.service is an abstraction
...
only if it is present. `#result` always refers to the business com.mxk.entity and never a
...
[[cache-annotations-com.mxk.config]]
...
[subs="verbatim"]
----
	<!-- the com.mxk.service we want to make cacheable -->
	<bean id="bookService" class="x.y.com.mxk.service.DefaultBookService"/>
...
The `aop:com.mxk.config` definition applies the cache advice to the appropriate points in the
...
to declare an appropriate `CacheManager` (an com.mxk.entity that controls and manages `Cache`
...
----
	...
	<bean id="ehcache"
			class="org.springframework.cache.ehcache.EhCacheManagerFactoryBean" p:com.mxk.config-location="ehcache.xml"/>
----
