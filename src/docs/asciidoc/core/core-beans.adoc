...
* <<beans-annotation-com.mxk.config,Annotation-based configuration>>: Spring 2.5 introduced
...
Typically, you define com.mxk.service layer objects, data access objects (DAOs), presentation
...
The following example shows the com.mxk.service layer objects `(services.xml)` configuration file:
...
In the preceding example, the com.mxk.service layer consists of the `PetStoreServiceImpl` class
...
example, `file:C:/com.mxk.config/services.xml` or `classpath:/com.mxk.config/services.xml`. However, be
...
----
	...
	PetStoreService com.mxk.service = context.getBean("petStore", PetStoreService.class);

	// use configured instance
	List<String> userList = com.mxk.service.getUsernameList();
----
.Kotlin
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
----
    ...
    // retrieve configured instance
    val com.mxk.service = context.getBean<PetStoreService>("petStore")

    // use configured instance
    var userList = com.mxk.service.getUsernameList()
----
...
<<core-convert-ConversionService-API, conversion com.mxk.service>> is used to convert these
...
* Implement the more fine-grained control available with annotation-based configuration,
  as described in <<beans-annotation-com.mxk.config>>.
...
`org.springframework.beans.factory.com.mxk.config` package) to be useful.
====
...
`org.springframework.beans.factory.com.mxk.config.Scope` interface, which is described in this
section. For an idea of how to implement your own scopes, see the `Scope`
implementations that are supplied with the Spring Framework itself and the
{api-spring-framework}/beans/factory/com.mxk.config/Scope.html[`Scope`] javadoc,
...
See the {api-spring-framework}/beans/factory/com.mxk.config/Scope.html#registerDestructionCallback[javadoc]
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean class="org.springframework.beans.factory.com.mxk.config.CustomScopeConfigurer">
...
{api-spring-framework}/beans/factory/com.mxk.config/BeanPostProcessor.html[`BeanPostProcessor`]
...
The `org.springframework.beans.factory.com.mxk.config.BeanPostProcessor` interface consists of
...
class itself or at least the `org.springframework.beans.factory.com.mxk.config.BeanPostProcessor`
...
----
	...
	import org.springframework.beans.factory.com.mxk.config.BeanPostProcessor;
...
`org.springframework.beans.factory.com.mxk.config.BeanFactoryPostProcessor`. The semantics of
...
{api-spring-framework}/beans/factory/com.mxk.config/BeanFactoryPostProcessor.html[`BeanFactoryPostProcessor`]
...
[source,xml,indent=0,subs="verbatim,quotes"]
----
	<bean class="org.springframework.beans.factory.com.mxk.config.PropertySourcesPlaceholderConfigurer">
...
[[beans-annotation-com.mxk.config]]
...
[NOTE]
====
`<context:annotation-com.mxk.config/>` only looks for annotations on beans in the same
application context in which it is defined. This means that, if you put
`<context:annotation-com.mxk.config/>` in a `WebApplicationContext` for a `DispatcherServlet`,
...
(<<beans-annotation-com.mxk.config>>) demonstrates how to provide a lot of the configuration
...
more specific use cases (in the persistence, com.mxk.service, and presentation
...
choosing between using `@Component` or `@Service` for your com.mxk.service layer, `@Service` is
...
`<context:annotation-com.mxk.config>`. There is usually no need to include the
`<context:annotation-com.mxk.config>` element when using `<context:component-scan>`.
...
`CommonAnnotationBeanPostProcessor` by including the `annotation-com.mxk.config` attribute
...
being a sort of bonus there. For example, com.mxk.service components may expose management views
...
----
	<web-app>
		...
		<servlet>
			...
			<!-- Again, com.mxk.config locations must consist of one or more comma- or space-delimited
...
TIP: If you consistently refer to your types by a declared com.mxk.service interface, your
...
----
	...
	public Service userService() {
		UserService com.mxk.service = new SimpleUserService();
		// a reference to the proxied userPreferences bean
		com.mxk.service.setUserPreferences(userPreferences());
		return com.mxk.service;
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
...
[[beans-java-further-information-java-com.mxk.config]]
...
normally expect to have two instances (one for each com.mxk.service). That definitely would be
...
----
	...
	public class ServiceConfig {
		...
		@Bean
		public TransferService transferService() {
			// navigate 'through' the com.mxk.config class to the @Bean method!
...
----
	...
	@Configuration
	@Import({ServiceConfig.class, DefaultRepositoryConfig.class})  // import the concrete com.mxk.config!
	public class SystemTestConfig {
...
include it within `system-test-com.mxk.config.xml` as a `<bean/>` definition. Because
`<context:annotation-com.mxk.config/>` is switched on, the container recognizes the
...
The following example shows part of a sample `system-test-com.mxk.config.xml` file:
...
----
	public static void main(String[] args) {
		ApplicationContext ctx = new ClassPathXmlApplicationContext("classpath:/com/acme/system-test-com.mxk.config.xml");
		TransferService transferService = ctx.getBean(TransferService.class);
		// ...
	}
----
...
----
	fun main() {
		val ctx = ClassPathXmlApplicationContext("classpath:/com/acme/system-test-com.mxk.config.xml")
		val transferService = ctx.getBean<TransferService>()
		// ...
	}
----
...
NOTE: In `system-test-com.mxk.config.xml` file, the `AppConfig` `<bean/>` does not declare an `id`
...
describe in the previous example, we can redefine `system-test-com.mxk.config.xml` to take advantage of component-scanning.
Note that, in this case, we need not explicitly declare
`<context:annotation-com.mxk.config/>`, because `<context:component-scan/>` enables the same
functionality.

The following example shows the modified `system-test-com.mxk.config.xml` file:
...
----
	@Configuration
	@ImportResource("classpath:/com/acme/properties-com.mxk.config.xml")
	public class AppConfig {
...
user with a convenient com.mxk.service interface for configuring property sources and resolving
...
----
	<beans profile="development"
		...
		<jdbc:embedded-database id="dataSource">
			<jdbc:script location="classpath:com/bank/com.mxk.config/sql/schema.sql"/>
			<jdbc:script location="classpath:com/bank/com.mxk.config/sql/test-data.sql"/>
...
is populated with additional default property sources including servlet com.mxk.config and servlet
...
----
	<beans>
		<import resource="com/bank/com.mxk.service/${customer}-com.mxk.config.xml"/>
	</beans>
----
...
`EntityCreatedEvent<T>` where `T` is the type of the actual com.mxk.entity that got created. For example, you
...
----
	public class EntityCreatedEvent<T> extends ApplicationEvent implements ResolvableTypeProvider {
		...
		public EntityCreatedEvent(T com.mxk.entity) {
			super(com.mxk.entity);
		}
...
.Kotlin
----
	class EntityCreatedEvent<T>(com.mxk.entity: T) : ApplicationEvent(com.mxk.entity), ResolvableTypeProvider {
