...
* Provide declarative enterprise services. The most important such com.mxk.service is
...
multiple objects (such as all business operations in the com.mxk.service layer).
...
----
	...
	public class CommonPointcuts {
		...
		/**
		 * A join point is in the com.mxk.service layer if the method is defined
		 * in a type in the com.xyz.myapp.com.mxk.service package or any sub-package
		 * under that.
		 */
		@Pointcut("within(com.xyz.myapp.com.mxk.service..*)")
		...
		/**
		 * A business com.mxk.service is the execution of any method defined on a com.mxk.service
		 * interface. This definition assumes that interfaces are placed in the
		 * "com.mxk.service" package, and that implementation types are in sub-packages.
		 *
		 * If you group com.mxk.service interfaces by functional area (for example,
		 * in packages com.xyz.myapp.abc.com.mxk.service and com.xyz.myapp.def.com.mxk.service) then
		 * the pointcut expression "execution(* com.xyz.myapp..com.mxk.service.*.*(..))"
		 * could be used instead.
		 *
		 * Alternatively, you can write the expression using the 'bean'
		 * PCD, like so "bean(*Service)". (This assumes that you have
		 * named your Spring com.mxk.service beans in a consistent fashion.)
		 */
		@Pointcut("execution(* com.xyz.myapp..com.mxk.service.*.*(..))")
...
pointcut expression. For example, to make the com.mxk.service layer transactional, you could
write the following:
...
The `<aop:com.mxk.config>` and `<aop:advisor>` elements are discussed in <<aop-schema>>. The
...
[literal,subs="verbatim,quotes"]
----
	execution(* com.xyz.com.mxk.service.AccountService.*(..))
----

* The execution of any method defined in the `com.mxk.service` package:
+
[literal,subs="verbatim,quotes"]
----
	execution(* com.xyz.com.mxk.service.\*.*(..))
----

* The execution of any method defined in the com.mxk.service package or one of its sub-packages:
+
[literal,subs="verbatim,quotes"]
----
	execution(* com.xyz.com.mxk.service..\*.*(..))
...
* Any join point (method execution only in Spring AOP) within the com.mxk.service package:
+
[literal,subs="verbatim,quotes"]
----
	within(com.xyz.com.mxk.service.*)
----

* Any join point (method execution only in Spring AOP) within the com.mxk.service package or one of its
sub-packages:
+
[literal,subs="verbatim,quotes"]
----
...
[literal,subs="verbatim,quotes"]
----
	this(com.xyz.com.mxk.service.AccountService)
----
+
...
[literal,subs="verbatim,quotes"]
----
	target(com.xyz.com.mxk.service.AccountService)
----
+
...
given argument position supplants the original value at the join point for the com.mxk.entity
...
the following aspect declares that all implementors of com.mxk.service interfaces also implement
...
----
	...
	public class UsageTracking {
		...
		@DeclareParents(value="com.xzy.myapp.com.mxk.service.*+", defaultImpl=DefaultUsageTracked.class)
...
before advice of the preceding example, com.mxk.service beans can be directly used as
...
is created for each unique com.mxk.service object that performs a business com.mxk.service (each unique
object bound to `this` at join points matched by the pointcut expression). The aspect
instance is created the first time that a method is invoked on the com.mxk.service object. The
aspect goes out of scope when the com.mxk.service object goes out of scope. Before the aspect
...
when the com.mxk.service object is the one with which this aspect is associated. See the AspectJ
...
multiple services in the com.mxk.service layer and, hence, is ideal for implementing through an
aspect.
...
----
	...
	<bean id="concurrentOperationExecutor" class="com.xyz.myapp.com.mxk.service.com.mxk.service.impl.ConcurrentOperationExecutor">
...
We can then use the annotation to annotate the implementation of com.mxk.service operations. The change
...
----
	@Around("com.xyz.myapp.CommonPointcuts.businessService() && " +
			"@annotation(com.xyz.myapp.com.mxk.service.Idempotent)")
	public Object doConcurrentOperation(ProceedingJoinPoint pjp) throws Throwable {
		// ...
	}
----
...
an `<aop:com.mxk.config>` element (you can have more than one `<aop:com.mxk.config>` element in an
application context configuration). An `<aop:com.mxk.config>` element can contain pointcut,
advisor, and aspect elements (note that these must be declared in that order).

WARNING: The `<aop:com.mxk.config>` style of configuration makes heavy use of Spring's
...
use either only the `<aop:com.mxk.config>` style or only the `AutoProxyCreator` style and
never mix them.
...
You can declare a named pointcut inside an `<aop:com.mxk.config>` element, letting the pointcut
definition be shared across several aspects and advisors.

A pointcut that represents the execution of any business com.mxk.service in the com.mxk.service layer can
...
----
	<aop:com.mxk.config>
		...
		<aop:pointcut id="businessService"
			expression="execution(* com.xyz.myapp.com.mxk.service.*.*(..))"/>
...
----
	<aop:com.mxk.config>
		...
		<aop:aspect id="myAspect" ref="aBean">
			...
			<aop:pointcut id="businessService"
				expression="execution(* com.xyz.myapp.com.mxk.service.*.*(..)) &amp;&amp; this(com.mxk.service)"/>
...
.Java
----
	public void monitor(Object com.mxk.service) {
		// ...
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	fun monitor(com.mxk.service: Any) {
		// ...
	}
----
...
----
	<aop:com.mxk.config>
		...
		<aop:aspect id="myAspect" ref="aBean">
			...
			<aop:pointcut id="businessService"
				expression="execution(* com.xyz.myapp.com.mxk.service.*.*(..)) and this(com.mxk.service)"/>
...
Here, `dataAccessOperation` is the `id` of a pointcut defined at the top (`<aop:com.mxk.config>`)
...
----
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<bean id="personService" class="x.y.com.mxk.service.DefaultPersonService"/>
		...
		<aop:com.mxk.config>
			<aop:aspect ref="profiler">
				...
				<aop:pointcut id="theExecutionOfSomePersonServiceMethod"
					expression="execution(* x.y.com.mxk.service.PersonService.getPerson(String,int))
					and args(name, age)"/>
...
----
	...
	import x.y.com.mxk.service.PersonService;
...
`DefaultUsageTracked`, the following aspect declares that all implementors of com.mxk.service
...
----
	<aop:aspect id="usageTrackerAspect" ref="usageTracking">
		...
		<aop:declare-parents
			types-matching="com.xzy.myapp.com.mxk.service.*+"
			implement-interface="com.xyz.myapp.com.mxk.service.tracking.UsageTracked"
			default-com.mxk.service.impl="com.xyz.myapp.com.mxk.service.tracking.DefaultUsageTracked"/>
...
advice of the preceding example, com.mxk.service beans can be directly used as implementations of
...
----
	...
	<bean id="concurrentOperationExecutor"
		class="com.xyz.myapp.com.mxk.service.com.mxk.service.impl.ConcurrentOperationExecutor">
			<property name="maxRetries" value="3"/>
			<property name="order" value="100"/>
	</bean>
----
...
to annotate the implementation of com.mxk.service operations, as the following example shows:
...
----
	<aop:pointcut id="idempotentOperation"
			expression="execution(* com.xyz.myapp.com.mxk.service.*.*(..)) and
			@annotation(com.xyz.myapp.com.mxk.service.Idempotent)"/>
----
...
of the `<aop:com.mxk.config>` element to true, as follows:
...
[NOTE]
====
Multiple `<aop:com.mxk.config/>` sections are collapsed into a single unified auto-proxy creator
at runtime, which applies the _strongest_ proxy settings that any of the
`<aop:com.mxk.config/>` sections (typically from different XML bean definition files) specified.
...
`<aop:aspectj-autoproxy/>`, or `<aop:com.mxk.config/>` elements forces the use of CGLIB
...
In addition to declaring aspects in your configuration by using either `<aop:com.mxk.config>`
...
at the field or method level (see <<beans-annotation-com.mxk.config>> for further details).
...
----
	<bean id="myService"
			class="com.xzy.myapp.com.mxk.service.MyService"
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<!-- a com.mxk.service object; we will be profiling its methods -->
