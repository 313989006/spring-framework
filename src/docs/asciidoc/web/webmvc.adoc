...
Spring MVC, as many other web frameworks, is designed around the front com.mxk.controller
...
(see <<mvc-container-com.mxk.config>>):
...
| Help the `DispatcherServlet` to invoke a handler mapped to a request, regardless of
  how the handler is actually invoked. For example, invoking an annotated com.mxk.controller
...
[[mvc-servlet-com.mxk.config]]
=== Web MVC Config
[.small]#<<web-reactive.adoc#webflux-framework-com.mxk.config, WebFlux>>#
...
In most cases, the <<mvc-com.mxk.config>> is the best starting point. It declares the required
...
----
	public class MyWebAppInitializer extends AbstractDispatcherServletInitializer {
		...
		protected WebApplicationContext createServletApplicationContext() {
			...
			cxt.setConfigLocation("/WEB-INF/spring/dispatcher-com.mxk.config.xml");
			return cxt;
		}
	...
	class MyWebAppInitializer : AbstractDispatcherServletInitializer() {
		...
		override fun createServletApplicationContext(): WebApplicationContext {
			return XmlWebApplicationContext().apply {
				setConfigLocation("/WEB-INF/spring/dispatcher-com.mxk.config.xml")
			}
		}
...
* The `WebApplicationContext` is searched for and bound in the request as an attribute
  that the com.mxk.controller and other elements in the process can use. It is bound by default
...
| Whether to throw a `NoHandlerFoundException` when no handler was found for a request.
  ...
  `@ExceptionHandler` com.mxk.controller method) and handled as any others.
...
See <<mvc-com.mxk.config-interceptors>> in the section on MVC configuration for examples of how to
...
declare it as an <<mvc-ann-com.mxk.controller-advice>> bean or configure it directly on
...
The <<mvc-com.mxk.config>> automatically declares built-in resolvers for default Spring MVC
...
configuration. The <<mvc-com.mxk.config>> provides a dedicated configuration API for
<<mvc-com.mxk.config-view-resolvers>> and for adding logic-less
<<mvc-com.mxk.config-view-com.mxk.controller, View Controllers>> which are useful for HTML template
rendering without com.mxk.controller logic.
...
The net effect is the same as if the com.mxk.controller had returned a `RedirectView`, but now
the com.mxk.controller itself can operate in terms of logical view names. A logical view
...
Note that, if a com.mxk.controller method is annotated with the `@ResponseStatus`, the annotation
...
See <<mvc-com.mxk.config-view-resolvers>> under <<mvc-com.mxk.config>> for configuration details.
...
* In `web.xml`, add a `"<multipart-com.mxk.config>"` section to the servlet declaration.
...
(see <<mvc-container-com.mxk.config>>) all filters are automatically registered for all dispatch
...
for each request. Other strategies at the com.mxk.controller level, described earlier, can avoid
...
`AbstractAnnotationConfigDispatcherServletInitializer` (see <<mvc-container-com.mxk.config>>)
...
[[mvc-com.mxk.controller]]
== Annotated Controllers
[.small]#<<web-reactive.adoc#webflux-com.mxk.controller, WebFlux>>#
...
The following example shows a com.mxk.controller defined by annotations:
...
[[mvc-ann-com.mxk.controller]]
=== Declaration
[.small]#<<web-reactive.adoc#webflux-ann-com.mxk.controller, WebFlux>>#

You can define com.mxk.controller beans by using a standard Spring bean definition in the
...
itself meta-annotated with `@Controller` and `@ResponseBody` to indicate a com.mxk.controller whose
...
In some cases, you may need to decorate a com.mxk.controller with an AOP proxy at runtime.
One example is if you choose to have `@Transactional` annotations directly on the
com.mxk.controller. When this is the case, for controllers specifically, we recommend
using class-based proxying. This is typically the default choice with controllers.
However, if a com.mxk.controller must implement an interface that is not a Spring Context
...
arguably, most com.mxk.controller methods should be mapped to a specific HTTP method versus
...
See <<mvc-com.mxk.config-path-matching>> in the configuration section.
...
com.mxk.controller mapped to `/person` is also implicitly mapped to `/person.{asterisk}`.
...
* `useSuffixPatternMatching(false)`, see <<mvc-com.mxk.config-path-matching, PathMatchConfigurer>>
* `favorPathExtension(false)`, see <<mvc-com.mxk.config-content-negotiation, ContentNegotiationConfigurer>>
...
`mediaTypes` property of <<mvc-com.mxk.config-content-negotiation,ContentNegotiationConfigurer>>.
...
See <<mvc-com.mxk.config-content-negotiation>>.
...
content types that a com.mxk.controller method produces, as the following example shows:
...
com.mxk.controller methods should be mapped to a specific HTTP method versus using `@RequestMapping`,
...
supported com.mxk.controller method arguments and return values.
...
The next table describes the supported com.mxk.controller method arguments. Reactive types are not supported
for any arguments.
...
The next table describes the supported com.mxk.controller method return values. Reactive types are
...
| A method with a `void` return type (or `null` return value) is considered to have fully
  ...
  an `@ResponseStatus` annotation. The same is also true if the com.mxk.controller has made a positive
...
| Alternative to `DeferredResult`, as a convenience (for example, when an underlying com.mxk.service
...
Some annotated com.mxk.controller method arguments that represent `String`-based request input (such as
...
If a URL is expected to contain matrix variables, the request mapping for a com.mxk.controller
...
<<mvc-com.mxk.config-path-matching>>. In the MVC XML namespace, you can set
...
query parameters or form data) to a method argument in a com.mxk.controller.
...
for such errors in the com.mxk.controller method, you can add a `BindingResult` argument immediately next
...
cases, you can inject the `Model` into the com.mxk.controller and access it directly or,
...
specific com.mxk.controller. This typically lists the names of model attributes or types of
...
until another com.mxk.controller method uses a `SessionStatus` method argument to clear the
...
(that is, outside the com.mxk.controller -- for example, by a filter) and may or may not be present,
...
`javax.servlet.http.HttpSession` into the com.mxk.controller method.

For temporary storage of model attributes in the session as part of a com.mxk.controller
...
`Model` should never be used if a com.mxk.controller method redirects. Instead, the com.mxk.controller
...
`Model` of the com.mxk.controller that serves the target URL.
...
Multipart requests can also be submitted from non-browser clients in a RESTful com.mxk.service
...
within the com.mxk.controller through an `Errors` or `BindingResult` argument,
as the following example shows:
...
You can use the <<mvc-com.mxk.config-message-converters>> option of the <<mvc-com.mxk.config>> to
...
----
	@PostMapping("/accounts")
	public void handle(HttpEntity<Account> com.mxk.entity) {
		// ...
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
...
----
	@PostMapping("/accounts")
	fun handle(com.mxk.entity: HttpEntity<Account>) {
		// ...
	}
----
...
all com.mxk.controller methods. This is the effect of `@RestController`, which is nothing more
...
`@ResponseBody` or `ResponseEntity` com.mxk.controller methods, you can use Jackson's
...
com.mxk.controller method. If you need to activate multiple views, you can use a composite interface.
...
A com.mxk.controller can have any number of `@ModelAttribute` methods. All such methods are
invoked before `@RequestMapping` methods in the same com.mxk.controller. A `@ModelAttribute`
method can also be shared across controllers through `@ControllerAdvice`. See the section on
<<mvc-ann-com.mxk.controller-advice>> for more details.
...
headers, cookies, and others) to the target type of com.mxk.controller method arguments.
* Format model object values as `String` values when rendering HTML forms.

`@InitBinder` methods can register com.mxk.controller-specific `java.bean.PropertyEditor` or
Spring `Converter` and `Formatter` components. In addition, you can use the
<<mvc-com.mxk.config-conversion,MVC com.mxk.config>> to register `Converter` and `Formatter`
...
com.mxk.controller-specific `Formatter` implementations, as the following example shows:
...
=== Exceptions
[.small]#<<web-reactive.adoc#webflux-ann-com.mxk.controller-exceptions, WebFlux>>#

`@Controller` and <<mvc-ann-com.mxk.controller-advice, @ControllerAdvice>> classes can have
`@ExceptionHandler` methods to handle exceptions from com.mxk.controller methods, as the following example shows:
...
com.mxk.controller or `@ControllerAdvice` class. This means a cause match on a higher-priority
...
| `HandlerMethod`
| For access to the com.mxk.controller method that raised the exception.
...
| A method with a `void` return type (or `null` return value) is considered to have fully
  handled the response if it also has a `ServletResponse` an `OutputStream` argument, or
  a `@ResponseStatus` annotation. The same is also true if the com.mxk.controller has made a positive
  `ETag` or `lastModified` timestamp check (see <<mvc-caching-etag-lastmodified>> for details).
...
[[mvc-ann-com.mxk.controller-advice]]
=== Controller Advice
[.small]#<<web-reactive.adoc#webflux-ann-com.mxk.controller-advice, WebFlux>>#
...
Spring MVC provides a mechanism to prepare links to com.mxk.controller methods. For example,
the following MVC com.mxk.controller allows for link creation:
...
akin to mock testing through proxies to avoid referring to the com.mxk.controller method by name, as the following example shows
...
Given the preceding com.mxk.controller, you can prepare a link from a JSP, as follows:
...
return values in com.mxk.controller methods and provide basic support for a single asynchronous
return value.
...
in the Servlet container, com.mxk.controller methods can wrap any supported com.mxk.controller method
...
The com.mxk.controller can produce the return value asynchronously, from a different thread -- for
...
A com.mxk.controller can wrap any supported return value with `java.util.concurrent.Callable`,
as the following example shows:
...
* The com.mxk.controller returns a `DeferredResult` and saves it in some in-memory
...
* The com.mxk.controller returns a `Callable`.
...
com.mxk.controller method returned the given value or as if it produced the given exception.
...
asynchronous support is built around that mechanism. When a com.mxk.controller returns a `DeferredResult`,
...
com.mxk.controller is mapped again but, rather than invoking it, the `DeferredResult` value is used
(as if the com.mxk.controller returned it) to resume processing.
...
asynchronous and <<mvc-ann-async-reactive-types>> as return values in com.mxk.controller methods.
...
types in com.mxk.controller method arguments (for example, `@RequestBody`, `@RequestPart`, and others),
...
stream from a com.mxk.controller, return `SseEmitter`, as the following example shows:
...
Spring MVC supports use of reactive client libraries in a com.mxk.controller (also read
...
reactive types from the com.mxk.controller method.
...
com.mxk.controller methods. We highly recommended configuring this property if you
stream with reactive types or have com.mxk.controller methods that return `Callable`, since
...
against conditional request headers. A com.mxk.controller can add an `ETag` header and `Cache-Control`
...
You can also make the check against conditional request headers in the com.mxk.controller,
as the following example shows:
...
for optimal performance. See the section on configuring <<mvc-com.mxk.config-static-resources>>.
...
[[mvc-com.mxk.config]]
== MVC Config
[.small]#<<web-reactive.adoc#webflux-com.mxk.config, WebFlux>>#
...
see <<mvc-com.mxk.config-advanced-java>> and <<mvc-com.mxk.config-advanced-xml>>.
...
and <<mvc-servlet-com.mxk.config>>.



[[mvc-com.mxk.config-enable]]
=== Enable MVC Configuration
[.small]#<<web-reactive.adoc#webflux-com.mxk.config-enable, WebFlux>>#
...
[[mvc-com.mxk.config-customize]]
=== MVC Config API
[.small]#<<web-reactive.adoc#webflux-com.mxk.config-customize, WebFlux>>#
...
[[mvc-com.mxk.config-conversion]]
=== Type Conversion
[.small]#<<web-reactive.adoc#webflux-com.mxk.config-conversion, WebFlux>>#

By default, formatters for various number and date types are installed, along with support
for customization via `@NumberFormat` and `@DateTimeFormat` on fields.

To register custom formatters and converters in Java com.mxk.config, use the following:
...
To do the same in XML com.mxk.config, use the following:
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<mvc:annotation-driven conversion-com.mxk.service="conversionService"/>
...
[[mvc-com.mxk.config-validation]]
=== Validation
[.small]#<<web-reactive.adoc#webflux-com.mxk.config-validation, WebFlux>>#
...
`Validated` on com.mxk.controller method arguments.
...
[[mvc-com.mxk.config-interceptors]]
=== Interceptors
...
[[mvc-com.mxk.config-content-negotiation]]
=== Content Types
[.small]#<<web-reactive.adoc#webflux-com.mxk.config-content-negotiation, WebFlux>>#
...
[[mvc-com.mxk.config-message-converters]]
=== Message Converters
[.small]#<<web-reactive.adoc#webflux-com.mxk.config-message-codecs, WebFlux>>#

You can customize `HttpMessageConverter` in Java configuration by overriding
{api-spring-framework}/web/servlet/com.mxk.config/annotation/WebMvcConfigurer.html#configureMessageConverters-java.util.List-[`configureMessageConverters()`]
(to replace the default converters created by Spring MVC) or by overriding
{api-spring-framework}/web/servlet/com.mxk.config/annotation/WebMvcConfigurer.html#extendMessageConverters-java.util.List-[`extendMessageConverters()`]
...
[[mvc-com.mxk.config-view-com.mxk.controller]]
=== View Controllers

This is a shortcut for defining a `ParameterizableViewController` that immediately
forwards to a view when invoked. You can use it in static cases when there is no Java com.mxk.controller
...
using the `<mvc:view-com.mxk.controller>` element:

[source,xml,indent=0,subs="verbatim,quotes"]
----
	<mvc:view-com.mxk.controller path="/" view-name="home"/>
----

If an `@RequestMapping` method is mapped to a URL for any HTTP method then a view
com.mxk.controller cannot be used to handle the same URL. This is because a match by URL to an
annotated com.mxk.controller is considered a strong enough indication of endpoint ownership so
...
splitting URL handling across an annotated com.mxk.controller and a view com.mxk.controller.



[[mvc-com.mxk.config-view-resolvers]]
=== View Resolvers
[.small]#<<web-reactive.adoc#webflux-com.mxk.config-view-resolvers, WebFlux>>#
...
[[mvc-com.mxk.config-static-resources]]
=== Static Resources
[.small]#<<web-reactive.adoc#webflux-com.mxk.config-static-resources, WebFlux>>#
...
[[mvc-com.mxk.config-path-matching]]
=== Path Matching
[.small]#<<web-reactive.adoc#webflux-com.mxk.config-path-matching, WebFlux>>#

You can customize options related to path matching and treatment of the URL.
For details on the individual options, see the
{api-spring-framework}/web/servlet/com.mxk.config/annotation/PathMatchConfigurer.html[`PathMatchConfigurer`] javadoc.
...
[[mvc-com.mxk.config-advanced-java]]
=== Advanced Java Config
[.small]#<<web-reactive.adoc#webflux-com.mxk.config-advanced-java, WebFlux>>#
...
[[mvc-com.mxk.config-advanced-xml]]
=== Advanced XML Config
