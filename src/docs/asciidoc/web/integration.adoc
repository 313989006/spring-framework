...
entry points into a server-side application, and it delegates to com.mxk.service objects
(facades) that are defined in a com.mxk.service layer to satisfy business-specific (and
presentation-technology agnostic) use cases. In Spring, these com.mxk.service objects, any other
...
`faces-com.mxk.config.xml`, but, at times, you may need to explicitly grab a bean.
