...
* <<webflux-com.mxk.controller>>: Consistent with Spring MVC and based on the same annotations
...
directly from Spring MVC com.mxk.controller methods. The greater the latency per call or the
...
<<webflux-com.mxk.config, WebFlux infrastructure>> and <<webflux-httphandler, run it>> with a few
lines of code.
...
`HttpMessageReader<Part>` instead. For example, in an annotated com.mxk.controller, use of
...
`WebHandler`. When using the <<webflux-com.mxk.config>>, registering a `WebFilter` is as simple
...
<<webflux-com.mxk.config>>, registering a `WebExceptionHandler` is as simple as declaring it as a
...
| Extension of `ResponseStatusExceptionHandler` that can also determine the HTTP status
  ...
  This handler is declared in the <<webflux-com.mxk.config>>.
...
<<webflux-com.mxk.config-message-codecs>>.
...
object — for example, a com.mxk.controller method with `@RequestBody byte[]`,
...
set all codecs, see <<webflux-com.mxk.config-message-codecs>>. On the client side, the limit for
...
Spring WebFlux, similarly to Spring MVC, is designed around the front com.mxk.controller pattern,
...
[[webflux-framework-com.mxk.config]]
=== WebFlux Config
[.small]#<<web.adoc#mvc-servlet-com.mxk.config, Web MVC>>#
...
However, in most cases, the <<webflux-com.mxk.config>> is the best starting point. It declares the
required beans and provides a higher-level configuration callback API to customize it.

NOTE: Spring Boot relies on the WebFlux com.mxk.config to configure Spring WebFlux and also provides
...
`HandlerResultHandler` implementations, all of which are declared in the <<webflux-com.mxk.config>>:
...
See also <<webflux-ann-com.mxk.controller-exceptions>> in the "`Annotated Controller`" section or
...
to your Spring configuration. <<webflux-com.mxk.config-view-resolvers, WebFlux Config>> provides a
...
The net effect is the same as if the com.mxk.controller had returned a `RedirectView` or
`Rendering.redirectTo("abc").build()`, but now the com.mxk.controller itself can
...
views through the <<webflux-com.mxk.config-view-resolvers, WebFlux Configuration>>. Default views are
always selected and used if they match the requested media type.




[[webflux-com.mxk.controller]]
== Annotated Controllers
[.small]#<<web.adoc#mvc-com.mxk.controller, Web MVC>>#
...
[[webflux-ann-com.mxk.controller]]
=== `@Controller`
[.small]#<<web.adoc#mvc-ann-com.mxk.controller, Web MVC>>#

You can define com.mxk.controller beans by using a standard Spring bean definition.
...
itself meta-annotated with `@Controller` and `@ResponseBody`, indicating a com.mxk.controller whose
...
because, arguably, most com.mxk.controller methods should be mapped to a specific HTTP method versus
...
The following table shows the supported com.mxk.controller method arguments.
...
The following table shows the supported com.mxk.controller method return values. Note that reactive
...
| A method with a `void`, possibly asynchronous (for example, `Mono<Void>`), return type (or a `null` return
  ...
  if the com.mxk.controller has made a positive ETag or `lastModified` timestamp check.
...
Some annotated com.mxk.controller method arguments that represent String-based request input (for example,
...
com.mxk.controller method, you need to add a URI variable to the path segment where matrix
...
com.mxk.controller. The following code snippet shows the usage:
...
to check for such errors in the com.mxk.controller method, you can add a `BindingResult` argument
...
another com.mxk.controller method uses a `SessionStatus` method argument to clear the storage,
as the following example shows:
...
`WebSession` into the com.mxk.controller method.
...
content. The best way to handle a file upload form (for example, from a browser) in a com.mxk.controller
...
You can also submit multipart requests from non-browser clients in a RESTful com.mxk.service
...
in the com.mxk.controller method by declaring the argument with an async wrapper and then using
...
You can use the <<webflux-com.mxk.config-message-codecs>> option of the <<webflux-com.mxk.config>> to
...
com.mxk.controller method by declaring the argument with an async wrapper and then using error
related operators:
...
com.mxk.controller method. Use a composite interface if you need to activate multiple views.
...
<<webflux-ann-com.mxk.controller-advice>> for more details.
...
<<webflux-com.mxk.config-conversion, WebFlux Java configuration>> to register `Converter` and
...
com.mxk.controller-specific `Formatter` instances, as the following example shows:
...
[[webflux-ann-com.mxk.controller-exceptions]]
=== Managing Exceptions
[.small]#<<web.adoc#mvc-ann-exceptionhandler, Web MVC>>#

`@Controller` and <<webflux-ann-com.mxk.controller-advice, @ControllerAdvice>> classes can have
`@ExceptionHandler` methods to handle exceptions from com.mxk.controller methods. The following
...
[[webflux-ann-com.mxk.controller-advice]]
=== Controller Advice
[.small]#<<web.adoc#mvc-ann-com.mxk.controller-advice, Web MVC>>#
...
against conditional request headers. A com.mxk.controller can add an `ETag` and `Cache-Control`
...
for optimal performance. See the section on configuring <<webflux-com.mxk.config-static-resources>>.




[[webflux-com.mxk.config]]
== WebFlux Config
[.small]#<<web.adoc#mvc-com.mxk.config, Web MVC>>#
...
<<webflux-com.mxk.config-advanced-java>>.



[[webflux-com.mxk.config-enable]]
=== Enabling WebFlux Config
[.small]#<<web.adoc#mvc-com.mxk.config-enable, Web MVC>>#

You can use the `@EnableWebFlux` annotation in your Java com.mxk.config, as the following example shows:
...
[[webflux-com.mxk.config-customize]]
=== WebFlux com.mxk.config API
[.small]#<<web.adoc#mvc-com.mxk.config-customize, Web MVC>>#
...
[[webflux-com.mxk.config-conversion]]
=== Conversion, formatting
[.small]#<<web.adoc#mvc-com.mxk.config-conversion, Web MVC>>#
...
[[webflux-com.mxk.config-validation]]
=== Validation
[.small]#<<web.adoc#mvc-com.mxk.config-validation, Web MVC>>#
...
mark it with `@Primary` in order to avoid conflict with the one declared in the MVC com.mxk.config.



[[webflux-com.mxk.config-content-negotiation]]
=== Content Type Resolvers
[.small]#<<web.adoc#mvc-com.mxk.config-content-negotiation, Web MVC>>#
...
[[webflux-com.mxk.config-message-codecs]]
=== HTTP message codecs
[.small]#<<web.adoc#mvc-com.mxk.config-message-converters, Web MVC>>#
...
[[webflux-com.mxk.config-view-resolvers]]
=== View Resolvers
[.small]#<<web.adoc#mvc-com.mxk.config-view-resolvers, Web MVC>>#
...
[[webflux-com.mxk.config-static-resources]]
=== Static Resources
[.small]#<<web.adoc#mvc-com.mxk.config-static-resources, Web MVC>>#
...
[[webflux-com.mxk.config-path-matching]]
=== Path Matching
[.small]#<<web.adoc#mvc-com.mxk.config-path-matching, Web MVC>>#

You can customize options related to path matching. For details on the individual options, see the
{api-spring-framework}/web/reactive/com.mxk.config/PathMatchConfigurer.html[`PathMatchConfigurer`] javadoc.
...
[.small]#<<web.adoc#mvc-com.mxk.config-advanced-java, Web MVC>>#
