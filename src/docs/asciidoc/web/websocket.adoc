...
[subs="verbatim,quotes"]
----
	import org.springframework.web.socket.com.mxk.config.annotation.EnableWebSocket;
	import org.springframework.web.socket.com.mxk.config.annotation.WebSocketConfigurer;
	import org.springframework.web.socket.com.mxk.config.annotation.WebSocketHandlerRegistry;
...
Neither of these mechanism makes it possible to use a single "`front com.mxk.controller`"
...
[.small]#<<web-reactive.adoc#webflux-websocket-server-com.mxk.config, WebFlux>>#
...
WebSocket Java com.mxk.config, as the following example shows:
...
that into Spring's `DefaultHandshakeHandler` through your WebSocket Java com.mxk.config.
...
[subs="verbatim,quotes"]
----
	import org.springframework.web.socket.com.mxk.config.annotation.EnableWebSocketMessageBroker;
	import org.springframework.web.socket.com.mxk.config.annotation.StompEndpointRegistry;
	...
	public class WebSocketConfig implements WebSocketMessageBrokerConfigurer {
		...
		@Override
		public void configureMessageBroker(MessageBrokerRegistry com.mxk.config) {
			com.mxk.config.setApplicationDestinationPrefixes("/app"); // <2>
			com.mxk.config.enableSimpleBroker("/topic", "/queue"); // <3>
		}
	}
----
...
[[websocket-stomp-server-com.mxk.config]]
=== WebSocket Server
...
com.mxk.controller can also do the same in response to HTTP requests, so a client can perform an
...
some `/app` destination, and a com.mxk.controller could return a value in response to that
...
a broker or a com.mxk.controller processes a given message first. If the goal is to be notified
...
<<web.adoc#mvc-ann-com.mxk.controller-advice, similar support>> available in Spring MVC.
...
re-established. For example, a Stock Quote com.mxk.service that broadcasts stock quotes can
...
cloud-based STOMP com.mxk.service).
...
After that, a com.mxk.controller can use a dot (`.`) as the separator in `@MessageMapping` methods,
...
you need to ensure that the authentication `ChannelInterceptor` com.mxk.config is ordered
...
inbound client messages and may be accessed from a com.mxk.controller method, as the following example shows:
...
time it is accessed from the com.mxk.controller and stores the instance in the WebSocket
...
The simplest form of server-side tests is to write com.mxk.controller unit tests. However,
this is not useful enough, since much of what a com.mxk.controller does depends on its
...
use it to send messages to be handled by com.mxk.controller methods.
