...
public class HttpComponentsAsyncClientHttpRequestFactoryTests extends AbstractAsyncHttpRequestFactoryTests {
	...
	public void customHttpAsyncClientUsesItsDefault() throws Exception {
		...
		assertThat(request.getHttpContext().getAttribute(HttpClientContext.REQUEST_CONFIG)).as("No custom com.mxk.config should be set with a custom HttpAsyncClient").isNull();
	}
	...
	public void defaultSettingsOfHttpAsyncClientLostOnExecutorCustomization() throws Exception {
		...
		assertThat(request.getHttpContext().getAttribute(HttpClientContext.REQUEST_CONFIG)).as("No custom com.mxk.config should be set with a custom HttpClient").isNull();
		...
		// No way to access the request com.mxk.config of the HTTP client so no way to "merge" our customizations
