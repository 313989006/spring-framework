...
public class HttpComponentsClientHttpRequestFactoryTests extends AbstractHttpRequestFactoryTests {
	...
	public void assertCustomConfig() throws Exception {
		...
		assertThat(config).as("Request com.mxk.config should be set").isNotNull();
		assertThat(RequestConfig.class.isInstance(config)).as("Wrong request com.mxk.config type" + config.getClass().getName()).isTrue();
	...
	public void mergeBasedOnCurrentHttpClient() throws Exception {
		...
		// Update the Http client so that it returns an updated  com.mxk.config
