...
public class ImportTests {
	...
	public void testProcessImportsWithAsm() {
		...
		DefaultListableBeanFactory beanFactory = new DefaultListableBeanFactory();
		beanFactory.registerBeanDefinition("com.mxk.config", new RootBeanDefinition(ConfigurationWithImportAnnotation.class.getName()));
		ConfigurationClassPostProcessor pp = new ConfigurationClassPostProcessor();
		pp.postProcessBeanFactory(beanFactory);
	...
	public void testImportAnnotationWithMultipleArgumentsResultingInOverriddenBeanDefinition() {
		DefaultListableBeanFactory beanFactory = new DefaultListableBeanFactory();
		beanFactory.registerBeanDefinition("com.mxk.config", new RootBeanDefinition(
	...
	public void reproSpr9023() {
		...
		assertThat(ctx.getBeanNamesForType(B.class)[0]).isEqualTo("com.mxk.config-b");
		assertThat(ctx.getBeanNamesForType(A.class)[0]).isEqualTo("com.mxk.config-a");
	}

	@Configuration("com.mxk.config-a")
	static class A { }

	@Configuration("com.mxk.config-b")
	@Import(A.class)
	static class B { }
