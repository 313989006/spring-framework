...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	 ...
	 */
	// 创建并返回新的 StandardEnvironment 标准环境
	// 子类可以重写此方法，以便提供自定义 ConfigurableEnvironment 可配置环境的
	 ...
	 * @see #getBeanFactory()
	 */
	// 如果此应用上下文已经可用，则将此上下文的内部 bean 工厂返回为 AutowireCapableBeanFactory
	@Override
	...
	// @return 上下文使用的内部 ApplicationEventMulticaster 应用事件多路广播器
	// @throws IllegalStateException 如果上下文未被初始化
	 ...
	 */
	/**
	* 返回此 ResourcePatternResolver 资源模式解析器
	 * 用于将多个资源的位置按照模式解析到资源实例中
	 * 默认是 org.springframework.core.io.support.PathMatchingResourcePatternResolver
	 * 支持 ant 风格的位置模式
	 * 可以在子类中重写，用于扩展解析策略，例如在 Web 环境中，在需要解决位置模式的时候
	 * 相反，调用上下文的 getResource()方法，它将委托给 ResourcePatternResolver
	...
	*/
	...
	public void refresh() throws BeansException, IllegalStateException {
		...
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				// 为容器的某些子类指定特殊的 BeanPost 事件处理器，钩子方法
				// 允许容器的子类去注册PostProcessor
				/** 
				* 这个方法在当前版本的 Spring 是 任何代码的
				 *  可能是 Spring 期待在后面的版本中去扩展的
				*/ 
				...
				// 方法扫描应用中所有 BeanDefinition 并注册到容器之中
				/**
				...
				*/
				...
				// 向容器注册Bean级别的后置处理器（注册 bean 的拦截器）
				...
				// 在单例Bean初始化之前预留给子类初始化其他特殊Bean 的口子
				onRefresh();
	...
	protected void prepareBeanFactory(ConfigurableListableBeanFactory beanFactory) {
		...
		// bean 表达式解析器 ，能够获取 bean 当中的属性在前台页面
		...
		// 对象和 String 类型的转换 ，比如将 <property ref ="indexDao"> 里的字符串 “indexDao” 转换成一个对象
		beanFactory.addPropertyEditorRegistrar(new ResourceEditorRegistrar(this, getEnvironment()));

		/**
		 * ApplicationContextAwareProcessor 这个类是干嘛的？
		 */
		// *********Spring 最最核心的代码***********
		// Configure the bean factory with context callbacks.
		// 添加该后置处理器的作用：当应用程序定义的 Bean 实现 ApplicationContextAware 接口时注入 ApplicationContext对象
		...
		// 修正依赖，这里是注册一些自动装配的特殊规则，比如是 BeanFactory class 接口的实现类，则在运行时修指定为当前 BeanFactory 实例
		...
		// ApplicationListenerDetector 的作用就是判断整个 Bean 是否是 ApplicationListener
		...
		// 注册默认environment 环境 bean
		/**
		* 意思是如果自定义的 Bean 中没有 名为 “systemProperties” 和 “systemEnvironment”   Bean
	 ...
	 */
	/**
	 * 钩子方法：并不是抽象的，所以子类可以选择性的去实现以满足自己的需求
	 * 在标准初始化后修改应用程序上下文的内部 Bean工厂
	 * 所有bean定义都将被加载，但是没有bean会被实例化
	 * 这允许某些应用上下文实现中注册特殊的 BeanPostProcessor
	 ...
	 */
	/**
	 * 实例化并调用所有已注册的 BeanFactoryPostProcessor 的 bean，如果已给出顺序，请按照顺序
	 * 必须在单实例实例化之前调用
	 */
	protected void invokeBeanFactoryPostProcessors(ConfigurableListableBeanFactory beanFactory) {
		// 这里的 beanFactory 为 DefaultListableBeanFactory 实例
		// getBeanFactoryPostProcessors() 获取的是自定义的 （就是程序猿自己写的，但是没有交给 Spring 管理的，也就是没有加上 @Component 注解的）
		/**
		* 这个地方需要注意 getBeanFactoryPostProcessors（） 是获取手动给 Spring 的 BeanFactoryPostProcessor
		 * 自定义的不仅仅是程序员自己写的，自己 的可以加 @Component 也可以不加
		 * 如果加了  @Component 注解，getBeanFactoryPostProcessors（）得不到，是 Spring 自己扫描的
		 *
		 * 为什么得不到？ 因为 getBeanFactoryPostProcessors（） 是直接获取一个 list
		 * 这个 list 是在 AnnotationConfigApplicationContext 被定义的
		 * 所谓的自定义就是你手动调用 AnnotationConfigApplicationContext.addBeanFactoryPostProcessor() 添加进去
		*/
	...
	protected void finishBeanFactoryInitialization(ConfigurableListableBeanFactory beanFactory) {
		// Initialize conversion com.mxk.service for this context.
		// 初始化此容器的转换器
	 ...
	 */
	/**
	 * 返回父上下文使用的内部 MessageSource
	 * @return 此上下文内部的 MessageSource （非null）
	 * @throws IllegalStateException 如果上下文未被实现
	 */
	 ...
	 */
	/**
	 * 如果父上下文也是 AbstractApplicationContext 抽象类，则返回父上下文的内部 MessageSource
	 * 否则，返回父上下文本身
	 */
	@Nullable
	...
	/**
	* 子类必须实现此方法来执行实际配置的加载
	 * 在任何其他初始化工作之前，可通过 refresh()方法调用此方法
	 * 子类要么创建一个新的bean工厂并持有对它的引用
	 * 要么返回一个它持有的 BeanFactory 实例
	 * 在后一种情况下，如果多次刷新上下文，它通常会抛出一个 IllegalStateException 异常
	*  @throws BeansException 如果bean 工厂的初始化失败
