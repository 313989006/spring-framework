...
/**
 *	用来创建代理的配置的父类，以确保所有代理创建者具有一致的属性
...
public class ProxyConfig implements Serializable {
	...
	// 标记是否对代理进行优化
	// true ：如果 Bean 有接口就直接使用  JDK 动态代理，没有接口就使用 CGLIB
	private boolean optimize = false;

	// 标记是否需要阻止通过该配置创建的代理对象转换为 Advised 类型，默认值为 false，表示代理对象可以被转换为 Advised 类型
	// Advised 接口其实就代表了被代理的对象（此接口是 Spring AOP 提供，它提供了方法可以对代理进行操作，比如移除一个切面之类的）
	...
	// 标记代理对象是否应该被 aop 框架通过  AopCOntext以 ThreadLocal 的形式暴露出去。
	// 当一个代理对象需要调用它【proxy本身】的另外一个代理方法时，这个属性将非常有用。默认是 false，以避免不必要的拦截
	boolean exposeProxy = false;

	// 标记是否需要冻结代理对象，即在代理对象生成之后，是否允许对其进行修改，默认为 false，
	// 当我们不希望调用方法转换成 Advised 对象之后的代理对象时，就可以设置为 true，给冻结上即可
